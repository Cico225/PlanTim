...
class ProjectController extends Controller
{
    ...
    public function storeTask(Request $request, $projectId)
    {
        ...
        // Handle kanban_column_id if provided
        if ($request->has('kanban_column_id') && Schema::hasColumn('tasks', 'kanban_column_id')) {
            $data['kanban_column_id'] = $request->input('kanban_column_id');
        }
    ...
    public function updateTask(Request $request, $projectId, $taskId)
    {
        ...
        if (isset($validatedData['status']) && $oldTask->status != $validatedData['status']) {
            ...
            // Update kanban_column_id if status changed and kanban_columns table exists
            if (Schema::hasTable('kanban_columns') && Schema::hasColumn('tasks', 'kanban_column_id')) {
                $newColumn = DB::table('kanban_columns')
                    ->where('project_id', $projectId)
                    ->where('status', $validatedData['status'])
                    ->first();
                
                if ($newColumn) {
                    $validatedData['kanban_column_id'] = $newColumn->id;
                }
            }
        }
        ...
        $data = ['updated_at' => now()];
        $taskColumns = ['title', 'description', 'status', 'priority', 'due_date', 'estimated_hours', 'actual_hours', 'kanban_column_id'];
     ...
     */
    public function getKanbanBoard(Request $request, $projectId)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
        ...
        if (!Schema::hasTable('kanban_columns')) {
            return response()->json(['columns' => [], 'message' => 'Kanban feature not available']);
        }

        $isAllProjects = ($projectId === 'all' || $projectId === 'All');
        $swimlane = $request->input('swimlane'); // 'assignee', 'priority', 'epic', null

        // For "all projects", use default columns based on status
        if ($isAllProjects) {
            $columns = collect([
                (object)['id' => 'all-todo', 'project_id' => null, 'name' => 'To Do', 'status' => 'todo', 'order' => 0, 'color' => '#94a3b8', 'wip_limit' => null, 'is_default' => true],
                (object)['id' => 'all-in-progress', 'project_id' => null, 'name' => 'In Progress', 'status' => 'in-progress', 'order' => 1, 'color' => '#3b82f6', 'wip_limit' => null, 'is_default' => true],
                (object)['id' => 'all-review', 'project_id' => null, 'name' => 'Review', 'status' => 'review', 'order' => 2, 'color' => '#f59e0b', 'wip_limit' => null, 'is_default' => true],
                (object)['id' => 'all-done', 'project_id' => null, 'name' => 'Done', 'status' => 'done', 'order' => 3, 'color' => '#10b981', 'wip_limit' => null, 'is_default' => true],
            ]);
        } else {
            // Get Kanban columns for this project
            ...
            if ($columns->isEmpty()) {
                $columns = $this->createDefaultKanbanColumns($projectId);
            }
        }

        // Get tasks for each column
        foreach ($columns as $column) {
            // Build select dynamically based on column existence
            $selectFields = ['tasks.*'];
            if (Schema::hasColumn('tasks', 'assigned_to')) {
                $selectFields[] = 'users.name as assigned_to_name';
            }
            ...
            // For "all projects", don't filter by project_id; otherwise filter by specific project
            if (!$isAllProjects) {
            ...
            // Filter by status or kanban_column_id
            // If using kanban_column_id mapping (only for specific projects, not for "all")
            if (!$isAllProjects && Schema::hasColumn('tasks', 'kanban_column_id')) {
                // Use kanban_column_id if available, otherwise use status
                $query->where(function($q) use ($column) {
                    $q->where('tasks.kanban_column_id', $column->id);
                    // Also include tasks with matching status but no explicit kanban_column_id (for backward compatibility)
                    if (Schema::hasColumn('tasks', 'status')) {
                        $q->orWhere(function($subQ) use ($column) {
                            $subQ->where('tasks.status', $column->status)
                                 ->whereNull('tasks.kanban_column_id');
                        });
                    }
                });
            } elseif (Schema::hasColumn('tasks', 'status')) {
                // Fall back to status-only filtering if kanban_column_id doesn't exist
                $query->where('tasks.status', $column->status);
            }
            ...
            // Apply swimlane grouping
            if ($swimlane) {
                switch ($swimlane) {
                    case 'assignee':
                        if (Schema::hasColumn('tasks', 'assigned_to')) {
                            $query->whereNotNull('tasks.assigned_to');
                        }
                        break;
                    case 'priority':
                        if (Schema::hasColumn('tasks', 'priority')) {
                            $query->orderByRaw("FIELD(tasks.priority, 'urgent', 'high', 'medium', 'low')");
                        }
                        break;
                }
            }

            // Order by position if column exists
            if (Schema::hasColumn('tasks', 'position')) {
                $query->orderBy('tasks.position', 'asc');
            } else {
            ...
            // Count tasks for WIP limit checking
            $column->tasks_count = $tasks->count();
            $column->tasks = $tasks;
            
            // Check WIP limit only if column exists
            if (Schema::hasColumn('kanban_columns', 'wip_limit')) {
                $column->wip_exceeded = $column->wip_limit !== null && $column->tasks_count > $column->wip_limit;
            } else {
                $column->wip_exceeded = false;
            }
        }
     ...
     */
    public function getKanbanColumns(Request $request, $projectId)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
     ...
     */
    public function createKanbanColumn(Request $request, $projectId)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
        ...
        // Check if status already mapped to a column in this project
        ...
        if ($exists) {
            return response()->json(['message' => 'Status already mapped to another column'], 422);
        }

        // Get max order if not provided
        $order = $request->input('order');
        if ($order === null) {
            $maxOrder = DB::table('kanban_columns')
                ->where('project_id', $projectId)
                ->max('order');
        ...
        $column = DB::table('kanban_columns')->find($columnId);
        ...
        $this->logActivity(
            ...
            $user->id,
            null,
            ['column_id' => $columnId, 'name' => $column->name],
            null,
            "Kanban column created: {$column->name}"
        );
     ...
     */
    public function updateKanbanColumn(Request $request, $projectId, $columnId)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
        ...
        $column = DB::table('kanban_columns')
            ->where('id', $columnId)
            ->where('project_id', $projectId)
            ->first();
        ...
        // Check if status change conflicts with another column
        if ($request->has('status') && $request->input('status') != $column->status) {
            $exists = DB::table('kanban_columns')
                ->where('project_id', $projectId)
                ->where('status', $request->input('status'))
                ->where('id', '!=', $columnId)
        ...
        $this->logActivity(
            ...
            (array)$column,
            null,
            "Kanban column updated: {$column->name}"
        );
     ...
     */
    public function deleteKanbanColumn(Request $request, $projectId, $columnId)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
        ...
        $tasksCount = DB::table('tasks')
            ...
            ->where(function($query) use ($column) {
                $query->where('status', $column->status);
                if (Schema::hasColumn('tasks', 'kanban_column_id')) {
                    $query->orWhere('kanban_column_id', $column->id);
                }
            })
            ->count();
        ...
        if ($tasksCount > 0) {
            return response()->json([
                'message' => 'Cannot delete column with tasks. Please move tasks first.',
                'tasks_count' => $tasksCount
            ], 422);
        }

        DB::table('kanban_columns')->where('id', $columnId)->delete();
        ...
        $this->logActivity(
            ...
            null,
            null,
            "Kanban column deleted: {$column->name}"
        );
    ...
    /**
     * Move task in Kanban board (drag & drop)
     */
    public function moveTaskInKanban(Request $request, $projectId, $taskId)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
        ...
        $validator = Validator::make($request->all(), [
            ...
            'previous_column_id' => 'nullable|exists:kanban_columns,id',
            'previous_position' => 'nullable|integer|min:0',
        ]);
        ...
        $taskQuery = DB::table('tasks')->where('id', $taskId);
        
        // Exclude soft-deleted tasks
        if (Schema::hasColumn('tasks', 'deleted_at')) {
            $taskQuery->whereNull('deleted_at');
        }
        ...
        $newColumnId = (int) $request->input('kanban_column_id');
        $newColumn = DB::table('kanban_columns')->where('id', $newColumnId)->where('project_id', $projectId)->first();
        
        if (!$newColumn) {
            return response()->json(['message' => 'Invalid column'], 404);
        }

        // Resolve task status from column (obavezno pri premještanju – kolona određuje status)
        $newStatus = null;
        if (Schema::hasColumn('kanban_columns', 'status')) {
            $colStatus = $newColumn->status ?? null;
            if ($colStatus !== null && $colStatus !== '') {
                $newStatus = $colStatus;
            }
        }
        if ($newStatus === null || $newStatus === '') {
            $nameToStatus = [
                ...
                'review' => 'review',
                'done' => 'done', 'završeno' => 'done', 'gotovo' => 'done',
            ];
            $name = is_string($newColumn->name ?? '') ? strtolower(trim($newColumn->name)) : '';
            $newStatus = $nameToStatus[$name] ?? 'todo';
        }

        // Check WIP limit
        if (isset($newColumn->wip_limit) && $newColumn->wip_limit !== null) {
            $tasksInColumn = DB::table('tasks')
                ->where('project_id', $projectId)
                ->where(function ($query) use ($newColumnId, $newStatus) {
                    if (Schema::hasColumn('tasks', 'kanban_column_id')) {
                        $query->where('kanban_column_id', $newColumnId);
                    }
                    if ($newStatus !== null && Schema::hasColumn('tasks', 'status')) {
                        $query->orWhere('status', $newStatus);
                    }
                })
                ->where('id', '!=', $taskId)
                ->count();

            if ($tasksInColumn >= (int) $newColumn->wip_limit) {
                return response()->json([
                    'message' => "WIP limit reached for column '{$newColumn->name}'. Limit: {$newColumn->wip_limit}",
                    'wip_limit' => $newColumn->wip_limit,
                    'current_count' => $tasksInColumn
                ], 422);
            }
        }
        ...
        // Update task – uvijek ažuriraj status prema koloni u koju je task premješten
        $updateData = [
            'updated_at' => now(),
        ];
        $updateData['status'] = $newStatus;

        // Set completed_at when moving to "done" status
        if ($newStatus === 'done' && Schema::hasColumn('tasks', 'completed_at')) {
            if (!$task->completed_at) {
                $updateData['completed_at'] = now();
            }
        } elseif ($newStatus !== 'done' && Schema::hasColumn('tasks', 'completed_at')) {
            // Clear completed_at if moving away from "done"
            $updateData['completed_at'] = null;
        }

        if (Schema::hasColumn('tasks', 'kanban_column_id')) {
            $updateData['kanban_column_id'] = $newColumnId;
        }

        $newPosition = $request->input('position');
        if ($newPosition !== null && Schema::hasColumn('tasks', 'position')) {
            // Update positions of other tasks in the new column
            $previousColumnId = $request->input('previous_column_id');
            $previousPosition = $request->input('previous_position');
            ...
            if ($previousColumnId == $newColumnId) {
                if ($previousPosition !== null && $newPosition !== null) {
                    if ($newPosition > $previousPosition) {
                        ...
                        DB::table('tasks')
                            ->where('project_id', $projectId)
                            ->where(function($query) use ($newColumn) {
                                if (Schema::hasColumn('tasks', 'kanban_column_id')) {
                                    $query->where('kanban_column_id', $newColumn->id);
                                } else {
                ...
                // Shift tasks in old column up (if applicable)
                if ($previousColumnId && Schema::hasColumn('tasks', 'kanban_column_id')) {
                    $previousColumn = DB::table('kanban_columns')->find($previousColumnId);
                    if ($previousColumn && $previousPosition !== null) {
                        DB::table('tasks')
                            ->where('project_id', $projectId)
                            ->where('kanban_column_id', $previousColumnId)
                            ->where('position', '>', $previousPosition)
                            ->where('id', '!=', $taskId)
                            ->decrement('position');
                    }
        ...
        try {
            ...
            Log::error('Failed to update task in Kanban move', [
                'error' => $e->getMessage(),
                'task_id' => $taskId,
                'project_id' => $projectId,
            ...
            return response()->json([
                'message' => 'Greška pri ažuriranju zadatka: ' . $e->getMessage()
            ], 500);
        }

        $task = DB::table('tasks')->find($taskId);

        // Log activity (wrap in try-catch to prevent failures from blocking the move)
        try {
            $this->logActivity(
                $projectId,
                $taskId,
                ...
                (array)$task,
                [
                    'from_column' => $request->input('previous_column_id'),
                    'to_column' => $newColumnId,
                    'position' => $newPosition
                ],
                "Task moved to column: {$newColumn->name}"
            );
        } catch (\Exception $e) {
            Log::warning('Failed to log activity for Kanban move', [
                'error' => $e->getMessage(),
                'task_id' => $taskId,
                'project_id' => $projectId
            ]);
        ...
        // Update project progress when status changes (wrap in try-catch)
        try {
            $this->updateProjectProgress($projectId);
        } catch (\Exception $e) {
            Log::warning('Failed to update project progress after Kanban move', [
                'error' => $e->getMessage(),
                'project_id' => $projectId
            ]);
    ...
    /**
     * Create default Kanban columns for a project
     */
    private function createDefaultKanbanColumns($projectId)
    {
        $defaultColumns = [
            ['name' => 'To Do', 'status' => 'todo', 'order' => 0, 'color' => '#94a3b8'],
            ['name' => 'In Progress', 'status' => 'in-progress', 'order' => 1, 'color' => '#3b82f6'],
            ['name' => 'Review', 'status' => 'review', 'order' => 2, 'color' => '#f59e0b'],
            ['name' => 'Done', 'status' => 'done', 'order' => 3, 'color' => '#10b981'],
        ];
        ...
        foreach ($defaultColumns as $column) {
            ...
            if (Schema::hasColumn('kanban_columns', 'color')) {
                $insertData['color'] = $column['color'];
            }
            if (Schema::hasColumn('kanban_columns', 'is_default')) {
                $insertData['is_default'] = true;
            }
            if (Schema::hasColumn('kanban_columns', 'wip_limit')) {
                $insertData['wip_limit'] = null;
            }
            
            $columnId = DB::table('kanban_columns')->insertGetId($insertData);
            $columnIds[] = $columnId;
        }

        return DB::table('kanban_columns')
