      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-white shadow-2xl dark:bg-dark-800"
          ...
          >
            <div className="relative overflow-hidden px-6 py-8 text-white">
              ...
              <div className="relative flex items-start justify-between gap-4">
                <div>
                  <motion.p
                    className="mb-1 text-xs font-semibold uppercase tracking-[0.2em] text-white/75"
                  ...
                  >
                    <span className="rounded-2xl bg-white/15 px-4 py-2 text-4xl font-black tracking-tight backdrop-blur-sm">
                      v{primary?.version ?? '1.0.0'}
                    </span>
                    {primary?.version_name && (
                      <span className="pb-1 text-sm font-medium text-white/85">
                        {primary.version_name}
                      </span>
                    )}
                  </motion.div>
                  {primary?.released_at && (
                    <p className="mt-2 text-sm text-white/70">{formatDate(primary.released_at)}</p>
                  )}
                </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
              {primary?.release_notes && (
                <motion.p
                  className="mb-5 rounded-xl bg-primary-50 px-4 py-3 text-sm leading-relaxed text-primary-900 dark:bg-primary-900/20 dark:text-primary-100"
              ...
              {loadingHistory ? (
                ...
                <ul className="space-y-3">
                  {changelog.map((item, index) => {
                    ...
                    return (
                      <motion.li
                        key={`${item}-${index}`}
                        className="flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 dark:border-dark-600 dark:bg-dark-900/40"
                        initial={{ opacity: 0, x: -16 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: 0.35 + index * 0.08, type: 'spring', stiffness: 260 }}
                      >
                        <motion.span
                          className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary-100 text-primary-600 dark:bg-primary-900/40 dark:text-primary-300"
                          initial={{ scale: 0 }}
                          animate={{ scale: 1 }}
                ...
                <p className="rounded-xl border border-dashed border-gray-200 px-4 py-6 text-center text-sm text-gray-500 dark:border-dark-600 dark:text-gray-400">
                  {t('version.noChanges')}
                </p>
              )}
                ...
                <div className="mt-8">
                  ...
                  <div className="space-y-3">
                    {olderVersions.map((entry, index) => (
                      ...
                      >
                        <details className="group rounded-xl border border-gray-100 dark:border-dark-600">
                        <summary className="cursor-pointer list-none px-4 py-3 text-sm font-medium text-gray-800 dark:text-gray-100">
                          <span className="inline-flex items-center gap-2">
                            <span className="rounded-lg bg-gray-100 px-2 py-0.5 font-mono text-xs dark:bg-dark-700">
                              v{entry.version}
                            </span>
                            {entry.version_name}
                            {entry.released_at && (
